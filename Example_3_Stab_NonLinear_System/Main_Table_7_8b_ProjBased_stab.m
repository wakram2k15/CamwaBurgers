...
total_start = tic;   % total time for whole run

for nl=1:4
    
    refinement_start = tic;   % time per refinement
   ...
   %%%%Construction of FE matrices  
    ...
    opts.tol = 1e-6;
    try
        [Vfull, Dfull] = eigs(Awh, E1, ne_compute, 'SM', opts);  % nfree x ne_compute
        [Zfull, Dfull2] = eigs(Awh', E1, ne_compute, 'SM', opts); % adjoint
    catch
