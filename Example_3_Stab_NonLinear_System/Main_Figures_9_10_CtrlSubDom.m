...
%         plot(Time, Ez(:,6), '-k');
     ...
     figure(2);
        Time=dt:dt:T;
        semilogy(Time, Ez(:,1), '-g'); hold on
        semilogy(Time, Ez(:,2), '-r');
        semilogy(Time, Ez(:,3), '-b');
        semilogy(Time, Ez(:,4), '-c');
%         semilogy(Time, Ez(:,5), '-m');
%         semilogy(Time, Ez(:,6), '-k');
        grid on;
          title('Evolution of stabilized solution  on log-scale against time $t$',...
              'Interpreter', 'latex')
        xlabel('Time $t$', 'Interpreter', 'latex')
...
%         plot(Time, Eu(:,6), '-k');
     ...
     figure(4);
        Time=dt:dt:T;
        semilogy(Time, Eu(:,1), '-g'); hold on
        semilogy(Time, Eu(:,2), '-r');
        semilogy(Time, Eu(:,3), '-b');
        semilogy(Time, Eu(:,4), '-c');
%         semilogy(Time, Eu(:,5), '-m');
%         semilogy(Time, Eu(:,6), '-k');
        grid on;
          title('Evolution of stabilizing control on log-scale against time $t$',...
              'Interpreter', 'latex')
        xlabel('Time $t$', 'Interpreter', 'latex')
        ylabel('$\|u_h(t)\|$ on log-scale', 'Interpreter', 'latex')
